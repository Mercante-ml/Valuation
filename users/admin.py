...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = ('cnpj', 'email', 'razao_social', 'is_staff', 'is_active', 'usage_count')
    # Campos pelos quais se pode pesquisar
    search_fields = ('cnpj', 'email', 'razao_social')
    ...
    fieldsets = UserAdmin.fieldsets + (
        ('Dados da Empresa', {'fields': ('razao_social', 'usage_count')}),
    )
    add_fieldsets = UserAdmin.add_fieldsets + (
        ('Dados da Empresa', {'fields': ('razao_social',)}),
    )
